 ...
 */
export async function mylarDownloadIssue(
	issueId: string,
	...
	try {
		...
		console.info(
			`Downloaded issue ${issueId} (CBZ): ${(data.length / 1024 / 1024).toFixed(2)} MB`,
		);
		return data;
	} catch (error) {
 ...
 */
export async function mylarGetSeriesArt(
	seriesId: string,
): Promise<Uint8Array | null> {
	try {
		const res = await fetch(
