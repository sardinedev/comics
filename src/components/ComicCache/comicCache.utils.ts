 ...
 */
export function parseIssueIdFromDownloadUrl(input: string | Request): string | null {
...
export async function openComicCache(): Promise<Cache | null> {
  try {
    if (typeof caches !== "undefined") return await caches.open(COMIC_CACHE_NAME);
  } catch { /* Cache API unavailable (HTTP, older browser, quota/state issue). */ }
  return null;
}
...
async function writeCachedComicMetadataSafely(
  ...
  } catch { /* Metadata sidecars are useful, but should not block reading. */ }
}
 ...
 */
async function getCachedArchiveSize(cache: Cache, request: Request): Promise<number> {
...
export async function listCachedComics(): Promise<CachedComic[]> {
  ...
  const archiveRequests = requests
    .map((request) => ({ request, issueId: parseIssueIdFromDownloadUrl(request) }))
    .filter((entry): entry is { request: Request; issueId: string } => entry.issueId !== null);

  const comics = await Promise.all(
    archiveRequests.map(async ({ request, issueId }) => {
      const metadata = await readCachedComicMetadata(issueId);
      const sizeBytes = metadata?.sizeBytes ?? await getCachedArchiveSize(cache, request);
      return {
        issueId,
        sizeBytes,
 ...
 */
export async function deleteCachedIssue(issueId: string): Promise<CacheDeleteResult> {
  const cache = await openComicCache();
...
export async function downloadIssueToCache(
  ...
  if (!response.ok) {
    ...
    throw new Error((body as { error?: string }).error ?? `Download failed (${response.status})`);
  }
