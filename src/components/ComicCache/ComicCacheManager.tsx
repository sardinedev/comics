...
function formatIssueTitle(comic: CachedComic): string {
  ...
  const issueNumber = metadata.issueNumber != null ? ` #${metadata.issueNumber}` : "";
...
export function ComicCacheManager() {
  ...
  function toggleAll() {
    setConfirmBulkDelete(false);
    setSelectedIds(allSelected ? new Set() : new Set(comics.map((comic) => comic.issueId)));
  }
  ...
  async function removeIssues(issueIds: string[]) {
    ...
    setComics((current) => current.filter((comic) => !issueIds.includes(comic.issueId)));
    setSelectedIds((current) => {
      const next = new Set(current);
  ...
  if (state === "loading") {
    return (
      <div class="border border-slate-800 bg-slate-900 p-6" role="status" aria-live="polite">
  ...
  if (state === "unsupported") {
    return (
      <div class="border border-slate-800 bg-slate-900 p-6">
        <p class="text-sm text-slate-400">Browser cache storage is unavailable in this context.</p>
      </div>
    );
  }
    ...
    <section class="flex flex-col gap-6">
      <div class="grid gap-px overflow-hidden border border-slate-800 bg-slate-800 sm:grid-cols-3">
        <div class="bg-slate-900 p-4">
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-600">Cached comics</p>
          <p class="mt-2 text-3xl font-black text-white">{comics.length}</p>
        </div>
        <div class="bg-slate-900 p-4">
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-600">Storage used</p>
          <p class="mt-2 text-3xl font-black text-white">{formatBytes(totalSize)}</p>
        </div>
        <div class="bg-slate-900 p-4">
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-600">Selected</p>
          <p class="mt-2 text-3xl font-black text-white">{formatBytes(selectedSize)}</p>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            {confirmBulkDelete ? "Confirm delete" : `Delete ${selectedIds.size || ""}`}
          </button>
        </div>
      </div>
      ...
      {comics.length === 0 ? (
        <div class="border border-slate-800 bg-slate-900 p-8">
          <p class="text-sm text-slate-500">No comics are cached in this browser.</p>
        </div>
      ) : (
        <ol class="divide-y divide-slate-800 border border-slate-800">
          {comics.map((comic) => (
            <li key={comic.issueId} class="grid gap-4 bg-slate-900 p-4 sm:grid-cols-[auto_56px_minmax(0,1fr)_auto] sm:items-center">
              ...
              <div class="min-w-0">
                ...
                <p class="mt-1 truncate text-xs text-slate-500">{formatIssueMeta(comic)}</p>
                <p class="mt-2 text-[10px] font-bold uppercase tracking-widest text-slate-600">
                  {formatBytes(comic.sizeBytes)}
                  {comic.metadata?.cachedAt ? ` · Cached ${comic.metadata.cachedAt.slice(0, 10)}` : ""}
                </p>
              </div>
