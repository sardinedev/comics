...
const { deleteCachedIssue, listCachedComics } = await import("./comicCache.utils");
...
describe("ComicCacheManager", () => {
  test("renders cached comics and deletes after confirmation", async () => {
    ...
    mockedDeleteCachedIssue.mockResolvedValue({ archiveDeleted: true, metadataDeleted: true });

    render(<ComicCacheManager />);

    await expect.element(page.getByRole("link", { name: "Saga #1" })).toBeInTheDocument();
    await expect.element(page.getByText(/Cached 2026-05-03/)).toBeInTheDocument();
    ...
    await expect.element(page.getByText("No comics are cached in this browser.")).toBeInTheDocument();
  });
  ...
  test("renders and deletes a sidecar-less cached comic", async () => {
    ...
    mockedDeleteCachedIssue.mockResolvedValue({ archiveDeleted: true, metadataDeleted: false });

    render(<ComicCacheManager />);

    await expect.element(page.getByRole("link", { name: "Issue legacy" })).toBeInTheDocument();
    ...
    await page.getByRole("button", { name: "Confirm delete Issue legacy" }).click();
  ...
  test("bulk deletes all selected cached comics after confirmation", async () => {
    ...
    expect(mockedDeleteCachedIssue.mock.calls.map(([issueId]) => issueId)).toEqual(["i1", "i2"]);
    await expect.element(page.getByText("No comics are cached in this browser.")).toBeInTheDocument();
  });
});
