...
import { formatIssueLabel, getDownloadActionCopy } from "./bulkDownload.utils";
import {
...
export function BulkDownload({ issues, downloadedIssues }: BulkDownloadProps) {
	...
	useEffect(() => {
		...
		(async () => {
			...
			setCachedIds(
				new Set(
					entries.filter((entry) => entry.cached).map((entry) => entry.issueId),
				),
			);
			setPhase("idle");
		})();
	...
	const cachedCount = downloadedIssues.filter((issue) =>
		cachedIds.has(issue.issueId),
	).length;
	...
	const downloadIssues = useCallback(
		async (targetIssues: ComicCacheMetadataInput[]) => {
				...
				>
					...
					<div class="flex flex-col p-2">
						...
						{failedIssues.length > 0 && phase !== "downloading" && (
							...
							>
								Retry {failedIssues.length} issue
