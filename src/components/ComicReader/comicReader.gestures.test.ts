...
describe("comicReader gestures", () => {
  ...
  test("calculates contained image dimensions", () => {
    expect(getContainedImageRect({ width: 400, height: 800 }, { width: 2000, height: 1000 })).toEqual({
      x: 0,
      y: 300,
      width: 400,
      height: 200,
  ...
  test("clamps pan translation within visible image bounds", () => {
    ...
    expect(clampZoomTranslation(999, 999, bounds)).toEqual({ translateX: 200, translateY: 0 });
    expect(clampZoomTranslation(-999, -999, bounds)).toEqual({ translateX: -200, translateY: 0 });
  });
