...
export function ComicReader({
	...
	const phase = useSignal<"downloading" | "extracting" | "ready">(
		"downloading",
	);
	...
	const lastStoredProgressRef = useRef<{
		issueId: string;
		currentPage: number;
		totalPages: number;
	} | null>(null);
	...
	function flushProgress() {
		...
		saveReaderProgress(issueId, currentPage.value + 1, pages.value.length);
		...
		if (ok) {
			...
			lastSavedPageRef.current = currentPage.value;
		}
	}
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				createdUrls = urls;
				pageRefs.current = new Array<HTMLDivElement | null>(urls.length).fill(
					null,
				);
				imageRefs.current = new Array<HTMLImageElement | null>(
					urls.length,
				).fill(null);
				pages.value = urls;
				const resolvedInitialPage = resolveInitialReaderPage(
					issueId,
					initialPage || 1,
					{ preferStored: preferStoredProgress },
				);
				const startPage = Math.max(
					0,
					Math.min(resolvedInitialPage - 1, urls.length - 1),
				);
				...
				error.value = isOfflineCacheMiss
					? "This issue is not cached on this device yet."
	...
	}, [issueId, initialPage, cacheMetadata, preferStoredProgress]);
	...
	useSignalEffect(() => {
		...
		const lastProgress = lastStoredProgressRef.current;
		if (
			lastProgress?.issueId === nextProgress.issueId &&
			lastProgress.currentPage === nextProgress.currentPage &&
			lastProgress.totalPages === nextProgress.totalPages
		)
			return;

		lastStoredProgressRef.current = nextProgress;
		saveReaderProgress(
			nextProgress.issueId,
			nextProgress.currentPage,
			nextProgress.totalPages,
		);
	});

	useEffect(() => {
		void syncPendingReaderProgress();
		const onOnline = () => {
			void syncPendingReaderProgress();
		};
	...
	function navigateBack() {
		flushProgress();
		const href = backHref ?? `/comic/${issueId}`;
	...
	function handlePointerDown(event: TargetedPointerEvent<HTMLDivElement>) {
		...
		const bounds =
			zoom.value.region && imageMetrics.value
		...
		activeGestureRef.current = {
			...
			edgeAtStart: bounds
				? getHorizontalPanEdge(zoom.value.translateX, bounds)
				: null,
		};
					...
					return (
						...
						>
							<img
								...
								data-current-page={isCurrentPage ? "true" : undefined}
								data-zoom-region={
									isCurrentPage ? (zoom.value.region ?? undefined) : undefined
								}
			...
			{nextIssue && nextIssueReadUrl && isLastPage.value && (
				<div class="pointer-events-none absolute inset-x-0 bottom-0 z-30 bg-gradient-to-t from-black/90 via-black/70 to-transparent px-4 pb-5 pt-16">
					<div class="pointer-events-auto mx-auto flex max-w-xl items-center justify-between gap-4 border-t border-slate-700 bg-black/80 px-4 py-3 backdrop-blur-sm">
						<div class="min-w-0">
							<p class="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-500">
								Next issue?
							</p>
			...
			{showHomeScreenHint.value && (
				<div class="pointer-events-auto absolute inset-x-4 bottom-16 z-30 rounded-xl bg-slate-900/95 p-4 shadow-xl">
					...
					<p class="text-sm text-slate-400">
						Tap <span class="font-medium text-white">Share</span> →{" "}
						<span class="font-medium text-white">Add to Home Screen</span> to
						read without browser chrome.
					</p>
					<button
						type="button"
						onClick={() => {
							showHomeScreenHint.value = false;
						}}
