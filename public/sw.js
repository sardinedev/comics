...
async function networkFirstCover(request) {
	try {
		...
		return (
			(await caches.match(request, { cacheName: COVER_CACHE })) ?? response
		);
	} catch (error) {
		const cached = await caches.match(request, { cacheName: COVER_CACHE });
		if (cached) return cached;
		throw error;
	}
}
...
async function isOfflineShellReady() {
	...
	return [...pageMatches, ...assetMatches].every(Boolean);
}
